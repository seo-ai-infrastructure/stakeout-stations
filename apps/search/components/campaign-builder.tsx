...
export default function CampaignBuilder({onClose,onSave,storage='browser'}:{onClose:()=>void;onSave:(c:Campaign)=>void|Promise<void>;storage?:'browser'|'account'}) {
  const dialog = useRef<HTMLDialogElement>(null);
  const [chosen,setChosen]=useState<string[]>([surfaces[0],surfaces[1]]);
  const [errors,setErrors]=useState<string[]>([]);const [busy,setBusy]=useState(false);
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>){
    ...
    setBusy(true);try{await onSave({...values,id:crypto.randomUUID(),createdAt:new Date().toISOString(),status:'draft'});}catch(error){setErrors([error instanceof Error?error.message:'Campaign could not be saved']);}finally{setBusy(false);}
  }
  return <dialog ref={dialog} onCancel={e=>{if(busy)e.preventDefault();else onClose();}} className="builder"><div className="dialog-heading"><div><span className="muted">Managed search observation</span><h2>New campaign</h2></div><button className="icon-button" disabled={busy} onClick={onClose} aria-label="Close campaign builder"><X/></button></div><form onSubmit={submit}>
    <fieldset><legend>01 / Business & market</legend><label>Business name<input name="business" required maxLength={160} placeholder="Stakeout Search" defaultValue={storage==='account'?'Stakeout Search':undefined} autoFocus/></label><label>Business website or Google Maps listing<input name="listing" type="url" required placeholder="https://stakeoutsearch.com" defaultValue={storage==='account'?'https://stakeoutsearch.com':undefined}/></label><label>Target city or neighborhood<input name="location" required maxLength={160} placeholder="Miami Beach, FL"/></label><label>Keywords <span className="muted">one per line</span><textarea name="keywords" required rows={3} placeholder={'car accident lawyer near me\npersonal injury lawyer'}/></label></fieldset>
    ...
    <footer className="dialog-footer"><span>{storage==='account'?'Saved to your workspace · no devices started':'Saved in this browser · no devices started'}</span><button disabled={busy} className="button primary" type="submit">{busy?'Saving…':'Save campaign draft'} <ArrowRight size={16}/></button></footer>
  </form></dialog>;
}
