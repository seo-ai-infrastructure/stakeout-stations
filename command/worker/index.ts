...
createServer((req,res)=>{if(req.url!=='/health'){res.writeHead(404);res.end();return;}const healthy=!enabled||!!db&&!lastError&&Date.now()-lastTick<300000;res.writeHead(healthy?200:503,{'Content-Type':'application/json'});res.end(JSON.stringify({status:!enabled?'disabled':healthy?'ready':'unavailable',configured:!!db,enabled,last_tick:new Date(lastTick).toISOString(),error:lastError?'Controller connection requires attention':null}));}).listen(Number(process.env.PORT||8080),'0.0.0.0');
