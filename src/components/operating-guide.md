...
3. Register the intended station explicitly with its DuoPlus image ID and target coordinates. Inventory synchronization does not automatically import every remote phone.
...
There is no fixed 20-second provider fleet pulse. Inventory is requested during fleet-sync jobs and as needed by eligible live operations; its Redis cache lasts up to 60 seconds. OFFLINE stations are excluded from the ordinary coordinate selection query. Consequently, a phone started by an external DuoPlus template is not guaranteed to wake this application automatically.
...
This is not a guaranteed 140-device service level, and there is no implemented two-imports-per-pulse budget because automatic fleet imports do not exist. A dark fleet produces no per-device coordinate commands, but the database supervisor and queue infrastructure remain active.
...
| Provider fleet pulse every 20 seconds | Missing. Database supervision is a separate clock. |
| Automatic import, two new phones per pulse | Missing. Registration is explicit. |
